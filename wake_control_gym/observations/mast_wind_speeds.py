...
class MastWindSpeeds(Observation):
    ...
    def __init__(self, simulator: Simulator, measurement_points: list[MeasurementPoint]) -> None:
        ...
        self.low = torch.full(
            (simulator.num_turbines,), MIN_WINDSPEED, device=simulator.device, dtype=simulator.dtype
        )
        self.high = torch.full(
            (simulator.num_turbines,), MAX_WINDSPEED, device=simulator.device, dtype=simulator.dtype
        )
